import { useState } from 'react'
import { initiateGoogleAuth } from '@services/authService'
import { cn } from '@utils/cn'

/**
 * Google OAuth Button
 * Initiates the Google OAuth flow when clicked
 */
export default function GoogleAuthButton({
  text = 'Continue with Google',
  className,
}) {
  const [isLoading, setIsLoading] = useState(false)

  const handleClick = async () => {
      setIsLoading(true)
      try {
        await initiateGoogleAuth()
      } catch (error) {
        setIsLoading(false)
        console.error('Google auth failed:', error)
      }
    }

  return (
    <button
      type="button"
      onClick={handleClick}
      disabled={isLoading}
      className={cn(
        'w-full flex items-center justify-center gap-3',
        'px-4 py-3 rounded-xl border-2 border-gray/20',
        'bg-white text-dark font-medium',
        'hover:border-gray/40 hover:shadow-md',
        'focus:outline-none focus:ring-2 focus:ring-primary/50',
        'transition-all duration-200',
        'hover:scale-[0.99] active:scale-[0.97]',
        'disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100',
        className
      )}
    >
      {isLoading ? (
        <div className="w-5 h-5 border-2 border-gray/30 border-t-primary rounded-full animate-spin" />
      ) : (
        <svg className="w-5 h-5" viewBox="0 0 24 24">
          <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="#FBBC05"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
          />
          <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
          />
        </svg>
      )}
      <span>{isLoading ? 'Redirecting...' : text}</span>
    </button>
  )
}

/**
 * Divider with "or" text
 */
export function AuthDivider({ text = 'or' }) {
  return (
    <div className="flex items-center gap-4 my-6">
      <div className="flex-1 h-px bg-gray/20" />
      <span className="text-sm text-gray font-medium">{text}</span>
      <div className="flex-1 h-px bg-gray/20" />
    </div>
  )
}
